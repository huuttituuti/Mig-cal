...
(() => {
  ...
  // src/other-mentions.ts
  init_live_reload();
  var otherMentions = [
    {
      "2022-08-05": "Ajovy aloitettu."
    },
    {
      "2023-03-23": "Noritren lopettamis kokeilu, ei onnistunut."
    },
    {
  ...
  // src/utils/days-in-month.ts
  init_live_reload();
  function daysInMonth(month, year) {
    return new Date(year, month, 0).getDate();
  }

  // src/utils/min-two-digits.ts
  init_live_reload();
  ...
  function calendar(years) {
    ...
    for (let i = 0; i < yearsArray.length; i++) {
      ...
      for (let i2 = 0; i2 < sections.length; i2++) {
        const index = minTwoDigits(Number([i2]) + 1);
        sections[i2].setAttribute("id", `${yearString}-${String(index)}`);
        const grid = sections[i2].querySelector("[calendar='grid']");
        const daysTotal = daysInMonth(index, yearString);
        const divModel = document.querySelector("[calendar='day-wrp']");
        let dayWrp;
        for (let i3 = 0; i3 < daysTotal; i3++) {
          dayWrp = divModel.cloneNode(true);
          dayWrp.id = `${yearString}-${index}-${minTwoDigits(i3 + 1)}`;
          dayWrp.querySelector("[calendar='day']").textContent = i3 + 1;
          grid.appendChild(dayWrp);
        }
        const days = sections[i2].querySelectorAll("[calendar='day-wrp']");
        for (let i3 = 0; i3 < days.length; i3++) {
          const day = days[i3].querySelector("[calendar='day']").textContent;
        }
        const fillerModel = document.querySelector("[calendar='filler']");
        let startDay = (/* @__PURE__ */ new Date(`${Number(yearString)}-${Number([i2]) + 1}-01`)).getDay();
        startDay = startDay === 0 ? 7 : startDay;
        for (let i3 = 0; i3 < startDay - 1; i3++) {
          if (startDay === -1) {
          } else if (startDay > 0) {
        ...
        sections[i2].removeAttribute("calendar");
      }
    }
    ...
    for (let i = 0; i < navMonthWrpChildren.length; i++) {
      ...
      const importantMention = otherMentions.find(
        (i2) => Object.keys(i2).toString().slice(0, 7) === attr
      );
      if (importantMention !== void 0) {
    ...
    currentYear.click();
    const todayMonth = ((/* @__PURE__ */ new Date()).getMonth() + 1).toString().padStart(2, "0");
          ...
          if (year !== null) {
            ...
            switch (month) {
              ...
              case "05":
                ...
                break;
              case "06":
                changingMonth.textContent = "Kes\xE4kuu";
                break;
              case "07":
                changingMonth.textContent = "Hein\xE4kuu";
                break;
              case "08":
  ...
  // src/utils/create-lines-prev.ts
  init_live_reload();

  // src/utils/get-dates.ts
  init_live_reload();
  function getDates(startDate, endDate) {
    const days = [], months = /* @__PURE__ */ new Set(), years = /* @__PURE__ */ new Set();
  ...
  // src/utils/create-lines-prev.ts
  function createLinesPrev(name, icon) {
    ...
    for (let i = 0; i < medicationValues.length; i++) {
      const nextDate = medicationValues.includes(medicationValues[i]) ? medicationValues[(medicationValues.indexOf(medicationValues[i]) + 1) % medicationValues.length] : null;
      if (nextDate?.amount === 0) {
        const allPrevDates = medicationValues.slice(0, medicationValues.indexOf(nextDate));
        if (allPrevDates.find((el) => el.amount === 0) === void 0) {
          ...
          if (medicationValues[medicationValues.length - 1].amount !== 0) {
            ...
            const todayMonth = ((/* @__PURE__ */ new Date()).getMonth() + 1).toString().padStart(2, "0");
            const todayDay = (/* @__PURE__ */ new Date()).getDate().toString().padStart(2, "0");
    ...
    for (let i = 0; i < groups.length; i++) {
      ...
      for (let i2 = 0; i2 < filteredDates.length; i2++) {
        const target = document.getElementById(filteredDates[i2]);
  ...
  // src/utils/create-med-stamps.ts
  init_live_reload();
  function createMedStamps(name, obj, icon, div) {
    const { stamp } = obj;
    if (stamp === true) {
  ...
  // src/utils/get-nth-days.ts
  init_live_reload();
  function getNthDays(startDay, number) {
    const start = Date.parse(startDay);
    const calculation = number * 24 * 60 * 60 * 1e3;
    const end = Date.now();
    const arr = [];
    arr.push(start);
    while (true) {
    ...
    const formatted = removed.map(
      (i) => `${new Date(i).getFullYear()}-${(new Date(i).getMonth() + 1).toString().padStart(2, "0")}-${new Date(i).getDate().toString().padStart(2, "0")}`
    );
    return formatted;
  }

  // src/utils/get-nth-days-by-number.ts
  init_live_reload();
  function getNthDaysByNumber(startDay, nthDay) {
    const dateYear = new Intl.DateTimeFormat("fi", { year: "numeric" }).format(/* @__PURE__ */ new Date());
    const dateMonth = new Intl.DateTimeFormat("fi", { month: "2-digit" }).format(/* @__PURE__ */ new Date());
    const dateDay = new Intl.DateTimeFormat("fi", { day: "2-digit" }).format(/* @__PURE__ */ new Date());
  ...
  // src/utils/total-mig-days.ts
  init_live_reload();
  function totalMigDays() {
  ...
  // src/infograms/heatmap.ts
  init_live_reload();
  var import_apexcharts = __toESM(require_apexcharts_common(), 1);

  // node_modules/.pnpm/apexcharts@3.44.0/node_modules/apexcharts/dist/locales/fi.json
  var fi_default = {
    name: "fi",
    options: {
      months: [
  ...
  function heatMap(migDaysTotal) {
    ...
    const chart = new import_apexcharts.default(div, options);
    chart.render();
  }

  // src/infograms/splineChart.ts
  init_live_reload();
  var import_apexcharts2 = __toESM(require_apexcharts_common(), 1);
  function splineChart(dateRange, migDaysTotal, prevMed, Ajovy) {
    ...
    const options = {
      series: [
        {
          name: "migreenip\xE4iv\xE4t (per kk)",
          data: migDaysTotal
        },
        {
          name: "noritren (mg)",
    ...
    const chart = new import_apexcharts2.default(div, options);
    chart.render();
  }
  ...
  window.Webflow.push(() => {
    ...
    const infogramAjovyDate = [
      ...
      { x: Date.parse(/* @__PURE__ */ new Date()), y: 1 }
    ];
      ...
      if (request.status >= 200 && request.status < 400) {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          if (data[i].other_med.length >= 1) {
            ...
            for (let i2 = 0; i2 < otherMeds.length; i2++) {
              const objects = Object.entries(otherMeds[i2]);
              Object.keys(otherMeds[i2]).forEach((key) => {
                const obj = otherMeds[i2][key];
                ...
                medsArray.push({
                  name: key,
                  icon,
                  amount: otherMeds[i2][key].amount_mg,
                  stamp: otherMeds[i2][key].stamp
                });
              });
            }
          }
        ...
        if (infogramNoritren[infogramNoritren.length - 1].y !== 0) {
          if (infogramNoritren[infogramNoritren.length - 1].x !== Date.parse(/* @__PURE__ */ new Date())) {
            const obj = {
              x: Date.parse(/* @__PURE__ */ new Date()),
        ...
        const todayDate = `${(/* @__PURE__ */ new Date()).getFullYear()}/${((/* @__PURE__ */ new Date()).getMonth() + 1).toString().padStart(2, "0")}`;
        ...
        infogramMigDaysTotal.map((i) => {
          if (i.x.replace("-", "/") === todayDate)
            todayDateIndex = infogramMigDaysTotal.indexOf(i);
        });
